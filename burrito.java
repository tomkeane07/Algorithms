...
public class burrito
{
   ...
   public static void main(String[] args){
    ...
    Destination origin = new Destination(0,"Burrito Bear", 53.38195, -6.59192, time);
      ...
      queue.add(delivery1);
      queue.add(delivery2);queue.add(delivery1);
      ...
      while(queue.size()>0){
        if(drone.x == origin.x && drone.y == origin.y)
         {
            //System.out.println("drone back at shop");
         }
   ...
   public static double SearchHorizon(Drone d,Destination p, ArrayList<Destination> queue,  double time, int lateCount)
   {
      ...
      double distanceDP = Distance(d.x, d.y, p.x, p.y);
      ...
      for(int i = 1; i<A; i++)
      {
         ...
         distanceDS = Distance(d.x, d.y, Stop.x, Stop.y);
         if(Distance(Stop.x, Stop.y, p.x, p.y) < distanceDP && distanceDS< distanceDP)
         {  l="";
            //flight to stop happens here
            //String l="";
              stops.add(Stop);
              queue.remove(i);
              A-=1;
              d.setLocation(Stop.x, Stop.y);
              travelled += distanceDS;// System.out.println(distanceDS);
              //System.out.println("___________________"+travelled);
              time += distanceDS*60/80;
              if(time>=Stop.orderT+0.3)
              {
                l="!";
                lateBy = time - Stop.orderT;
                lateCount+=1;
               // System.out.println("LATE by "+lateBy*10+" minutes");

              }
              i-=1;
              //System.out.println(d.x +", "+d.y);
              //System.out.println(Stop.address+l);
      ...
      distanceDS = Distance(d.x, d.y, p.x, p.y); //System.out.println(distanceDS);
      ...
      //System.out.println(p.address+l);
